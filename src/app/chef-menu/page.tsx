...
import { Plus, ChefHat, CheckCircle2, Circle, Wallet, Settings, UtensilsCrossed } from 'lucide-react';
...
export default function ChefMenuPage() {
  ...
  const checkChef = async () => {
    ...
    try {
      const [{ data: row }, { count }] = await Promise.all([
        supabase
          .from('user_profiles')
          .select('role, vendor_onboarding_complete, full_name, username, bio, avatar_url, cover_url, location, delivery_enabled, delivery_fee, stripe_account_id, stripe_onboarding_complete, stripe_charges_enabled, stripe_payouts_enabled')
          .eq('id', user.id)
          .single(),
        supabase
          .from('meals')
      ...
      if (!row.vendor_onboarding_complete) {
  ...
  const readiness = getChefReadiness({ ...profileRow, mealCount });
      ...
      <div className="max-w-3xl mx-auto px-4 py-4 space-y-5">
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
          ...
          <div className="w-full h-2 rounded-full bg-muted overflow-hidden mb-4">
            <div className="h-full bg-primary rounded-full" style={{ width: `${readiness.percent}%` }} />
          </div>
          <div className="space-y-3">
            {readiness.items.map((item) => (
              <div key={item.key} className="flex items-center justify-between gap-3 rounded-xl border border-border/60 p-3">
                <div className="flex items-center gap-3">
                  {item.complete ? <CheckCircle2 className="w-5 h-5 text-green-500" /> : <Circle className="w-5 h-5 text-muted-foreground" />}
                  <span className="text-sm text-foreground">{item.label}</span>
                </div>
                {!item.complete && (
                  <Link href={item.ctaHref} className="text-xs font-700 text-primary hover:underline">{item.ctaLabel}</Link>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <Link href="/edit-profile" className="rounded-2xl border border-border bg-card p-4 hover:border-primary/30 transition-colors">
            <p className="text-sm font-700 text-foreground">Complete vendor profile</p>
            <p className="text-xs text-muted-foreground mt-1">Edit business name, bio, photos, and location.</p>
          </Link>
          <Link href="/chef-menu" className="rounded-2xl border border-border bg-card p-4 hover:border-primary/30 transition-colors">
            <p className="text-sm font-700 text-foreground">Add first meal</p>
            <p className="text-xs text-muted-foreground mt-1">Build your menu so customers can order.</p>
          </Link>
          <button className="rounded-2xl border border-border bg-card p-4 text-left">
            <p className="text-sm font-700 text-foreground">Connect payout</p>
            <p className="text-xs text-muted-foreground mt-1">Stripe setup will live here for chef payouts.</p>
          </button>
        </div>
      </div>
    </AppLayout>
