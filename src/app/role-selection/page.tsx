...
export default function RoleSelectionPage() {
  ...
  const { user, loading: authLoading, refreshProfile, getCurrentUser } = useAuth();
  ...
  useEffect(() => {
    if (authLoading || user || recoveringUser) return;
  ...
  }, [authLoading, getCurrentUser, recoveringUser, user]);
  ...
  const handleContinue = async () => {
    ...
    if (!activeUser) {
      const missingUserMessage = 'Your session expired. Please sign in again.';
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4 py-12">
      ...
      <div className="w-full max-w-2xl">
        ...
        {/* Error */}
        {error && (
          <div className="mb-4 px-4 py-3 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-sm text-center space-y-2">
            <p>{error}</p>
            {!user && !authLoading ? (
              <Link href="/login" className="inline-flex items-center justify-center rounded-xl border border-destructive/20 bg-background px-3 py-2 text-xs font-semibold text-foreground hover:bg-muted transition-colors">
                Back to sign in
              </Link>
            ) : null}
          </div>
        ...
        <button
          ...
          disabled={!selectedRole || loading || authLoading || recoveringUser || !user}
          ...
          suppressHydrationWarning
        >
          {loading || authLoading || recoveringUser ? (
